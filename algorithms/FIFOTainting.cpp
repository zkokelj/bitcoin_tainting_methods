...
AlgorithmMetrics FIFOTainting(DB* db, std::string txid_n, bool writeToFile, std::ofstream& nongraphfile, DB* offchainDB)
{
    ...
    if(writeToFile)
    {
        myfile.open ("./results/fifo-" + txid_n + ".txt");
    }

    // check if offchainDB exists
	std::ofstream offChainFile;
	std::string offChainFileName = "./results/_offchain_fifo-" + txid_n + ".txt";
    ...
    while(toCheck.size() > 0)
    {
        ...
        // If we are checking for offchainDB -> write to file.
        if(offchainDB)
		{
			std::string val;
			...
			if(s.ok())
			{
				...
				//std::cout << "KNOWN ADDRESS REACHED" << val << std::endl;
				continue;
			}
		}
    ...
    nongraphfile << "fifo," + txid_n << "," << duration.count() << "," << metrics.unspentOutputsTainted << "," << metrics.unspentTaintedAmount  << ", " << startingValue.value();
